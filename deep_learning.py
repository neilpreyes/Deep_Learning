...
# SPECIFICATION: Use deep learning in order to classify fashion items
...
for n_hidden_values in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n_neurons_values in n_neurons:                      #looking or the best parameters w.r.t the number of neurons
        for l_rate_values in l_rate:                  #looking or the best parameters w.r.t the learning rate

            #build the model for each combination by calling the function:
            #model = build_model()
            model = build_model(n_hidden_values, n_neurons_values, len(class_names), l_rate_values)
            ...
            if accuracy >  highestAccuracy:
                ...
                best_model = model
                best_n_hidden = n_hidden_values
                best_n_neuron = n_neurons_values
                best_l_rate = l_rate_values

            print("Highest accuracy so far: " + str(highestAccuracy))
            print("Parameters: " + "Number of Hidden Layers: " + str(best_n_hidden) + ",number of neurons: " + str(best_n_neuron) + ",learning rate: " + str(best_l_rate))
            print()
